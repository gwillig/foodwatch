...
def current_streak(seq):
    """
    The the length of the current streak
    :param seq(list):
    :return:
        lenght_current_streak(int) e.g. 4
    @Doctest
    >>> current_streak([  4, 19, 20,21,22,25,19, 20,21,22])
    1
    >>> current_streak([ 0, 1, 2,  4,6])
    3
    >>> current_streak([ 0, 1, 2,  3,6])
    4
    """
    '#2.Step: Loop for the seq'
    current_sequence_length = 1
